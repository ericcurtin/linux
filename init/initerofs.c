 ...
 */
static bool __initdata initerofs_detected;

/*
 * Whether initerofs has successfully mounted the root filesystem.
 * __initdata is correct: both the writer (initerofs_try_mount) and the
 * sole reader (initerofs_root_mounted, called from prepare_namespace) are
 * __init functions that run before init sections are freed.
 */
static bool __initdata initerofs_mounted;
...
/*
 * Try to mount EROFS directly from the initrd memory region,
 * bypassing the ramdisk entirely.
 *
 * Creates a regular file in rootfs whose read_folio reads from
 * initrd memory on demand.  EROFS mounts from this file using
 * its file-backed mode.
 *
 * Returns true on success (root filesystem is mounted at /root).
 */
bool __init initerofs_try_mount(void)
{
	...
	/* Mount EROFS from the memory-backed file */
	...
	/*
	 * Remove the backing file from rootfs now that EROFS holds its own
	 * reference to the inode via the superblock.  The dentry is unlinked
	 * but the inode (and thus the initrd memory backing store) stays live
	 * for as long as the EROFS superblock holds its file reference.
	 * After pivot_root the old rootfs tmpfs is detached anyway, but
	 * unlinking here keeps the rootfs namespace tidy and avoids confusion
	 * if the old rootfs is ever inspected during debugging.
	 */
	...
	printk(KERN_INFO
	       "VFS: Mounted root (erofs filesystem) readonly on device %u:%u.\n",
...
bool __init initerofs_root_mounted(void)
{
