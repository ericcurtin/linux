 ...
 *
 * Usage:
 * - Create an EROFS image: mkfs.erofs -zlz4 initerofs.img rootfs/
 * - Use initerofs.img as your initramfs (bootloader loads it as usual)
...
/*
 * Check if the initramfs contains an EROFS filesystem.
...
bool __init initerofs_detect(void)
{
	...
	if (magic == EROFS_SUPER_MAGIC_V1) {
		pr_info("initerofs: detected EROFS format in initramfs\n");
		return true;
	}
...
/*
 * Mount the EROFS image from initramfs memory as the root filesystem.
...
int __init initerofs_mount_root(void)
{
	...
	pr_info("initerofs: mounting EROFS from initramfs at 0x%lx (size %lu bytes)\n",
		initrd_start, size);
	 ...
	 * initramfs memory region. This avoids any memory copy - EROFS reads the
	 * data directly from where the bootloader placed it.
	 *
	 * Benefits over file-backed approach:
	 * 1. Zero-copy: No need to write initramfs to a backing file
